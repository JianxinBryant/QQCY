  ...
  <body>
	<div class="container-fluid">
		...
		<c:forEach items="${orderInfoMap }" var="order">
			<jsp:setProperty name="dateValue" property="time" value="${order.key.o_createtime}"/>
			...
			<div class="col-md-8 orderkuang" >
				...
				<table class="biaoge table-hover">
					...
					<c:forEach items="${order.value }" var="orderInfo">
						...
						<tr class="biaogeti">
							<td>
							<div class="shucai">
								<img class="shucaiphoto img-rounded" src="${orderInfo.vegetable.v_photo}">
							</div>
							</td>
							<td>
								<span class="shucaimingcheng">${orderInfo.vegetable.v_name }</span><br><br>
								<span class="shucaimiaoshu">${orderInfo.vegetable.v_title } </span>
							</td>
				...
				<div class="main-foot">
					<div>
						<h4 id="main-foot-left">订单创建时间：<fmt:formatDate value="${dateValue}" pattern="yyyy-dd-MM HH:mm"/></h4>
					</div>
						...
						<h3>总金额：${order.key.o_sumprice }</h3>
							<c:choose>
								...
								<c:when test="${order.key.o_status == '已完成'  }">
									<h3><span class="label label-success" id="jiesuananniu" >订单完成</span></h3>
								</c:when>
								...
								<c:when test="${order.key.o_status == '关闭'  }">
									<h3><span class="label label-Default" id="jiesuananniu" >关闭</span></h3>
								</c:when>
								<c:when test="${order.key.o_status == '已退款'  }">
									<h3><span class="label label-warning" id="jiesuananniu" >已退款</span></h3>
								</c:when>
							</c:choose>
