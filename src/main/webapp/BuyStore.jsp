...
<body>
			...
			<table class="">
				...
				<c:forEach items="${shoppingCartInfos }" var="cartInfo">
					<tr>
								...
								<c:choose>
									<c:when test="${cartInfo.shi_status=='false'}">
										<input type="checkbox" name="cart_check"
											id="${cartInfo.vegetable.v_id }" onchange="updateCheckbox(${cartInfo.vegetable.v_id })" />
									</c:when>
									<c:otherwise>
										<input type="checkbox" name="cart_check"
											id="${cartInfo.vegetable.v_id }" onchange="updateCheckbox(${cartInfo.vegetable.v_id })" checked="checked"/>
									</c:otherwise>
								</c:choose>
								</div>
						...
						<td>
							<div>
								<label id="info3${cartInfo.vegetable.v_id }"><fmt:formatNumber value="${cartInfo.vegetable.v_price }" type="currency" pattern="0.00"/></label>
							</div>
						</td>
						...
						<td><span id="v_id${cartInfo.vegetable.v_id }" style="display: none">${cartInfo.vegetable.v_id}</span>
						</td>
	...
	<div >
		...
		<div  class="info">
			...
			总金额：<span id="finalprice" style="color: red">${shoppingCart.sh_sumprice }</span>
		</div>
